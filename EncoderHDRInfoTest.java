Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/media/src/android/mediav2/cts/EncoderHDRInfoTest.java blob:0b 4ed 23 bbf1015ea581b32ddc214c5688edbf3d0[file][log][blame]
/*
 ...
 * HDR Metadata is an aid for a display device to show the content in an optimal
 * manner. It
 * contains the HDR content and mastering device properties that are used by the
 * display device
 * to map the content according to its own color gamut and peak brightness. This
 * information can
 * be part of container and/or elementary stream. If the encoder is configured
 * with hdr metadata,
 * then it is expected to place this information in the elementary stream as-is.
 * If a muxer is
 * configured with hdr metadata then it is expected to place this information in
 * container as-is.
        ...
        for (String mediaType : HDRMediaTypes) {
            for (int maxBFrame : maxBFrames) {
                ...
                exhaustiveArgsList.add(new Object[] { mediaType, getVideoEncoderCfgParams(mediaType,
                        profile, maxBFrame),
                        String.format("%dkbps_%dx%d_%s_%s_%d-bframes", 512,
                                352, 288, "yuvp010", "hdrstaticinfo", maxBFrame),
                ...
                exhaustiveArgsList.add(new Object[] { mediaType, getVideoEncoderCfgParams(mediaType,
                        profile, maxBFrame),
                        String.format("%dkbps_%dx%d_%s_%s_%d-bframes", 512,
                                352, 288, "yuvp010", "hdrdynamicinfo", maxBFrame),
                        null, HDR_DYNAMIC_INFO });
            }
        }
