Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/media/src/android/mediav2/cts/EncoderProfileLevelTest.java blob:e79ed54dfb8d0d630e3de9b5d55164a1121f2379[file][log][blame]
/*
...
/**
 * EncoderProfileLevelTest validates the profile, level information advertised
 * by the component
 * in its codec capabilities. The test sets profile and level keys in media
 * format and uses it
 * during encoder configuration. Upon successful configuration, frames are
 * queued for encoding
 * (byte buffer mode) and the encoded output (bitstream) is expected to contain
 * the same profile
 * that was used during configure. The level shall be at least the input
 * configured level.
 * <p>
 * NOTE: The test configures level information basing on standard guidelines,
 * not arbitrarily so
 * encoders are expected to maintain at least the input configured level
 * <p>
 * The test parses the bitstream (csd or frame header) and determines profile
 * and level
 * information. This serves as reference for further validation. The test checks
 * if the output
 * format returned by component contains same profile and level information as
 * the bitstream. The
 * output of encoder is muxed and is extracted. The extracted format is expected
 * to contain same
 * profile and level information as the bitstream.
 * <p>
 * As per cdd, if a device contains an encoder capable of encoding a
 * profile/level combination
 * then it should contain a decoder capable of decoding the same profile/level
 * combination. This
 * is verified.
 * <p>
 * If device implementations support encoding in a media type, then as per cdd
...
public class EncoderProfileLevelTest extends EncoderProfileLevelTestBase {
    ...
    private static EncoderConfigParams[] getVideoEncoderCfgParams(String mediaType, int bitRate,
        ...
        for (int profile : profiles) {
            if (maxBframe != 0) {
                if (mediaType.equals(MediaFormat.MIMETYPE_VIDEO_AVC) && (profile == AVCProfileBaseline
    ...
    private boolean shallSupportProfileAndLevel(EncoderConfigParams cfg) {
        CddRequirements requirement = Objects.requireNonNull(CDD_REQUIREMENTS_MAP.get(cfg.mMediaType));
    ...
    public void testValidateProfileLevel() throws IOException, InterruptedException {
        ...
        // TODO(b/280510792): Remove the following once level can be configured
        // correctly in
        ...
        boolean cddSupportedMediaType = CDD_REQUIREMENTS_MAP.get(mMediaType) != null;
        {
            ...
            MediaCodecInfo.CodecCapabilities codecCapabilities = mCodec.getCodecInfo()
            ...
            for (EncoderConfigParams cfg : mEncCfgParams) {
                ...
                if (trackFormat != null)
                    checkIfTrackFormatIsOk(trackFormat);
