Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/tests/media/codec/src/android/media/codec/cts/VideoCodecTestBase.java blob:654896 b505317e06ba06ccc29745b966753ef7be[file][log][blame]
/*
 ...
 * target values.
 * The stream is later decoded by the decoder to verify frames are decodable and
 * to
...
public class VideoCodecTestBase {
    ...
    protected static final String SDCARD_DIR = Environment.getExternalStorageDirectory().getAbsolutePath();
    ...
    // Video bitrate type - should be set to OMX_Video_ControlRateConstant from
    // OMX_Video.h
    ...
    protected ArrayList<EncoderOutputStreamParameters> getDefaultEncodingParameterList(
        ...
        ArrayList<EncoderOutputStreamParameters> outputParameters = new ArrayList<EncoderOutputStreamParameters>(
                numCodecs);
    ...
    /**
     * Packs YUV420 frame by moving it to a smaller size buffer with stride and
     * slice
     ...
     *
     * @param inputIvfFilename  The name of the IVF file containing encoded
     *                          bitsream.
    ...
    /**
     * Helper function to return InputStream from either fully specified filename
     * (if set)
     ...
     *
     * Function feedInput() is used to send raw video frame to the encoder input.
     * When encoder
    ...
    protected class MediaEncoderAsync extends Thread {
        ...
        // Flag indicating if input frame was consumed by the encoder in feedInput()
        // call.
        ...
        private MediaCodec.Callback mCallback=new MediaCodec.Callback(){@Override public void onInputBufferAvailable(MediaCodec codec,int index){if(mHelper==null){Log.e(TAG,"async helper not available");return;}byte[]encFrame=mHelper.getInputFrame();boolean inputEOS=(encFrame==null);int encFrameLength=0;int flags=0;if(inputEOS){flags=MediaCodec.BUFFER_FLAG_END_OF_STREAM;}else{encFrameLength=encFrame.length;ByteBuffer byteBuffer=mCodec.getInputBuffer(index);byteBuffer.put(encFrame);byteBuffer.rewind();mInPresentationTimeUs=(mInputFrameIndex*1000000)/mFrameRate;Log.v(TAG,"Enc"+mId+". Frame in # "+mInputFrameIndex+". InTime: "+(mInPresentationTimeUs+500)/1000);mInputFrameIndex++;}mCodec.queueInputBuffer(index,0, // offset
        encFrameLength, // size
        mInPresentationTimeUs,flags);}@Override public void onOutputBufferAvailable(MediaCodec codec,int index,MediaCodec.BufferInfo info){if(mHelper==null){Log.e(TAG,"async helper not available");return;}MediaEncoderOutput out=new MediaEncoderOutput();out.buffer=new byte[info.size];ByteBuffer outputBuffer=mCodec.getOutputBuffer(index);outputBuffer.get(out.buffer,0,info.size);mOutPresentationTimeUs=info.presentationTimeUs;String logStr="Enc"+mId+". Frame # "+mOutputFrameIndex;if((info.flags&MediaCodec.BUFFER_FLAG_CODEC_CONFIG)!=0){logStr+=" CONFIG. ";}if((info.flags&MediaCodec.BUFFER_FLAG_SYNC_FRAME)!=0){logStr+=" KEY. ";if(!mInitialSyncFrameReceived){mInitialSyncFrameReceived=true;}}if((info.flags&MediaCodec.BUFFER_FLAG_END_OF_STREAM)!=0){logStr+=" EOS. ";}logStr+=" Size: "+info.size;logStr+=". InTime: "+(mInPresentationTimeUs+500)/1000+". OutTime: "+(mOutPresentationTimeUs+500)/1000;Log.v(TAG,logStr);if(!mInitialSyncFrameReceived&&(info.flags&MediaCodec.BUFFER_FLAG_CODEC_CONFIG)==0){throw new RuntimeException("Non codec_config_frame before first sync.");}if(info.size>0){mOutputFrameIndex++;out.inPresentationTimeUs=mInPresentationTimeUs;out.outPresentationTimeUs=mOutPresentationTimeUs;}MediaFormat format=codec.getOutputFormat(index);if(format.containsKey(MediaFormat.KEY_VIDEO_QP_AVERAGE)){int avgQp=format.getInteger(MediaFormat.KEY_VIDEO_QP_AVERAGE);
        // Copy per-frame avgQp to sequence level buffer
        mHelper.saveAvgQp(avgQp);}mCodec.releaseOutputBuffer(index,false);out.flags=info.flags;out.outputGenerated=true;if(mHelper.saveOutputFrame(out)){
        // output EOS
        signalCompletion();}}@Override public void onError(MediaCodec codec,CodecException e){Log.e(TAG,"onError: "+e+", transient "+e.isTransient()+", recoverable "+e.isRecoverable()+", error "+e.getErrorCode());}@Override public void onOutputFormatChanged(MediaCodec codec,MediaFormat format){Log.i(TAG,"onOutputFormatChanged: "+format.toString());}};
    ...
    protected VideoEncodeOutput encode(
        ...
        if (streamParams.encodingStatisticsLevel != MediaFormat.VIDEO_ENCODING_STATISTICS_LEVEL_NONE) {
        ...
        while (!sawOutputEOS) {
            ...
            // If codec is not ready to accept input/poutput - wait for buffer ready
            // callback
    ...
    protected ArrayList<ArrayList<MediaCodec.BufferInfo>> encodeSimulcast(
        ...
        ArrayList<ArrayList<MediaCodec.BufferInfo>> bufferInfos = new ArrayList<ArrayList<MediaCodec.BufferInfo>>(
                numEncoders);
        ...
        while (!sawOutputEOSTotal) {
            ...
            for (int i = 0; i < numEncoders; i++) {
                ...
                if (inputConsumed[i] && !sawInputEOS[i]) {
                    ...
                    // Convert YUV420 to NV12 if necessary
                    if (codecProperties[i].colorFormat != CodecCapabilities.COLOR_FormatYUV420Planar) {
     ...
     * mKeyFrames array will contain the position of key frames in the encoded
     * stream and
    ...
    protected VideoEncodingStatistics computeEncodingStatistics(int encoderId,
        ...
        if (keyFrames > 1) {
            statistics.mAverageKeyFrameInterval = statistics.mKeyFrames.get(keyFrames - 1)
                    - statistics.mKeyFrames.get(0);
            statistics.mAverageKeyFrameInterval = Math
                    .round((float) statistics.mAverageKeyFrameInterval / (keyFrames - 1));
            for (int j = 1; j < keyFrames; j++) {
                int keyFrameInterval = statistics.mKeyFrames.get(j) - statistics.mKeyFrames.get(j - 1);
                statistics.mMaximumKeyFrameInterval = Math.max(statistics.mMaximumKeyFrameInterval, keyFrameInterval);
                statistics.mMinimumKeyFrameInterval = Math.min(statistics.mMinimumKeyFrameInterval, keyFrameInterval);
            }
    ...
    protected ArrayList<VideoEncodingStatistics> computeSimulcastEncodingStatistics(
        ...
        for (int i = 0; i < numCodecs; i++) {
            VideoEncodingStatistics currentStatistics = computeEncodingStatistics(i, bufferInfos.get(i));
