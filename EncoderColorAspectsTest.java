Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/media/src/android/mediav2/cts/EncoderColorAspectsTest.java blob:31 a46ded1aaa2db61c0cfa8254d65f573f51f5dc[file][log][blame]
/*
...
/**
 * Color Primaries, Color Standard and Color Transfer are essential information
 * to display the
 * decoded YUV on an RGB display accurately. These 3 parameters can be signalled
 * via containers
 * (mp4, mkv, ...) and some video standards also allow signalling this
 * information in elementary
 * stream. Avc, Hevc, Av1, ... allow signalling this information in elementary
 * stream, vpx relies
 * on webm/mkv or some other container for signalling.
 * <p>
 * If the encoder is configured with color aspects, then it is expected to place
 * this information
 * in the elementary stream as-is if possible. The same goes for container as
 * well. The test
 * validates this.
 * <p>
 * Hybrid log gamma transfer characteristics are applicable for high bit depth
 * profiles. Standard
 * gamma curve characteristics are applicable for standard dynamic ranges. The
 * test doesn't
 * exhaustively try all combinations of primaries, standard, transfer on all
 * encoding profiles.
 * SDR specific characteristics are restricted to sdr profiles and HLG/HDR
 * specific profiles are
...
public class EncoderColorAspectsTest extends CodecEncoderTestBase {
    ...
    private static void prepareArgsList(List<Object[]> exhaustiveArgsList, String[] mediaTypes,
        ...
        // Assuming all combinations are supported by the standard which is true for
        // AVC, HEVC, AV1,
        // VP8 and VP9.
        int[] maxBFrames = { 0, 2 };
    ...
    protected void queueEOS() throws InterruptedException {
        if (mActiveEncCfg.mColorFormat != COLOR_FormatSurface) {
            ...
            if (!mAsyncHandle.hasSeenError() && !mSawInputEOS) {
                ...
                mSawInputEOS = true;
                if (ENABLE_LOGS)
                    Log.d(LOG_TAG, "signalled end of stream");
            }
        }
    }
    ...
    protected void doWork(int frameLimit) throws IOException, InterruptedException {
        if (mActiveEncCfg.mColorFormat != COLOR_FormatSurface) {
            ...
            while (!mAsyncHandle.hasSeenError() && !mSawInputEOS &&
                ...
                if (ENABLE_LOGS)
                    Log.d(LOG_TAG, "inputSurface swapBuffers");
    ...
    /**
     * ColorAspects are passed to the encoder at the time of configuration. The
     * encoder is
     * expected to pass this information to outputFormat() so that muxer can use
     * this information
     * to populate color metadata. If the bitstream is capable of capturing color
     * metadata
     * losslessly then encoder is also expected to use this information during
     * bitstream
     * generation. Although a given media type can be muxed using many containers,
     * the test does
     * not use all available ones. Instead the most preferred one is selected.
     * vpx streams are muxed using webm writer and others are muxed using mp4
     * writer.
     * Briefly, the test checks OMX/c2 framework, plugins, encoder, muxer ability to
     * SIGNAL color
     * metadata.
     */
        ...
        }
        {
            ...
            if (mActiveEncCfg.mColorFormat == COLOR_FormatSurface) {
                ...
                assertTrue("Surface is not valid \n" + mTestConfig + mTestEnv,
                        mInpSurface.isValid());
                mEGLWindowInpSurface = new InputSurface(mInpSurface, false, mActiveEncCfg.mInputBitDepth == 10);
            ...
            // if color metadata can also be signalled via elementary stream then verify if
            // the
