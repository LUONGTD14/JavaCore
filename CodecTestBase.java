Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/media/common/src/android/mediav2/common/cts/CodecTestBase.java blob:c2aa4b042b8933a31c6b583c176f5e6bc7af5c37[file][log][blame]
/*
...
/**
 * This class comprises of routines that are generic to media codec component
 * trying and testing.
 * <p>
 * A media codec component essentially processes input data to generate output
 * data. The
 * component uses a set of input and output buffers. At a simplistic level, the
 * client requests
 * (or receive) an empty input buffer, fills it up with data and sends it to the
 * codec for
 * processing. The codec uses up the data and transforms it into one of its
 * empty output
 * buffers. Finally, the client asks (or receive) a filled output buffer,
 * consume its contents and
 * release it back to the codec.
 * <p>
 * The type of data that a component receives and sends is dependent on the
 * component. For
 * instance video encoders expect raw yuv/rgb data and send compressed data. A
 * video decoder
 * receives compressed access-unit and sends reconstructed yuv. But the
 * processes surrounding
 * this enqueue and dequeue remain common to all components. Operations like
 * configure, requesting
 * the component for an empty input buffer, feeding the component a filled input
 * buffer,
 * requesting the component for an output buffer, releasing the processed output
 * buffer, Sending
 * state transition commands, waiting on component to send all outputs, closing
 * the component and
 * releasing the resources, ... remain more or less identical to all components.
 * The routines
 * that manage these generic processes are maintained by this class. Not only
 * the methods that
 * are responsible for component trying but also methods that test its
 * functionality are covered
 * here. A video component is expected to give same number of outputs as inputs.
 * The output
 * buffer timestamps of an audio component or a decoder component is expected to
 * be strictly
 * increasing. The routines that enforce these generic rules of all components
 * at all times are
 * part of this class. Besides these, mediaType specific constants, helper
 * utilities to test
 * specific components or mediaTypes are covered here.
 * <p>
 * enqueueInput and dequeueOutput are routines responsible for filling the input
 * buffer and
 * handing the received output buffer respectively. These shall be component
 * specific, hence they
 * are abstract methods. Any client intending to use this class shall implement
 * these methods
 * basing on the component under test.
 * <p>
 * In simple terms, the CodecTestBase is a wrapper class comprising generic
 * routines for
...
public abstract class CodecTestBase {
    ...
    public static final boolean IS_AT_LEAST_T = ApiLevelUtil.isAtLeast(Build.VERSION_CODES.TIRAMISU);
    // TODO(b/248315681) Remove codenameEquals() check once devices return correct
    // version for U
    ...
    public static final boolean FIRST_SDK_IS_AT_LEAST_T = ApiLevelUtil.isFirstApiAtLeast(Build.VERSION_CODES.TIRAMISU);
    public static final boolean VNDK_IS_AT_LEAST_T = SystemProperties.getInt("ro.vndk.version",
            Build.VERSION_CODES.CUR_DEVELOPMENT) >= Build.VERSION_CODES.TIRAMISU;
    public static final boolean VNDK_IS_BEFORE_U = SystemProperties.getInt("ro.vndk.version",
            Build.VERSION_CODES.CUR_DEVELOPMENT) < Build.VERSION_CODES.UPSIDE_DOWN_CAKE;
    public static final boolean BOARD_SDK_IS_AT_LEAST_T = SystemProperties.getInt("ro.board.api_level",
            Build.VERSION_CODES.CUR_DEVELOPMENT) >= Build.VERSION_CODES.TIRAMISU;
    ...
    public static final String HDR_STATIC_INFO = "00 d0 84 80 3e c2 33 c4 86 4c 1d b8 0b 13 3d 42 40 a0 0f 32 00 10 27 df 0d";
    public static final String HDR_STATIC_INCORRECT_INFO = "00 d0 84 80 3e c2 33 c4 86 10 27 d0 07 13 3d 42 40 a0 0f 32 00 10 27 df 0d";
    ...
    static final int[] VP9_HDR_PROFILES = combine(VP9_HLG_PROFILES,
            combine(VP9_HDR10_PROFILES, VP9_HDR10_PLUS_PROFILES));
    ...
    static final int[] HEVC_HDR_PROFILES = combine(HEVC_HLG_PROFILES,
            combine(HEVC_HDR10_PROFILES, HEVC_HDR10_PLUS_PROFILES));
    ...
    static final int[] AV1_HDR_PROFILES = combine(AV1_HLG_PROFILES,
            combine(AV1_HDR10_PROFILES, AV1_HDR10_PLUS_PROFILES));
    ...
    public static final Context CONTEXT = InstrumentationRegistry.getInstrumentation().getTargetContext();
    public static final int MAX_DISPLAY_HEIGHT_CURRENT = Arrays
            .stream(CONTEXT.getSystemService(DisplayManager.class).getDisplays())
            ...
            .getPhysicalHeight();
    public static final int MAX_DISPLAY_WIDTH_CURRENT = Arrays
            .stream(CONTEXT.getSystemService(DisplayManager.class).getDisplays())
            .map(Display::getSupportedModes)
            .flatMap(Stream::of)
            .max(Comparator.comparing(Display.Mode::getPhysicalHeight))
            .orElseThrow(() -> new RuntimeException("Failed to determine max height"))
            .getPhysicalWidth();
    public static final int MAX_DISPLAY_WIDTH_LAND = Math.max(MAX_DISPLAY_WIDTH_CURRENT, MAX_DISPLAY_HEIGHT_CURRENT);
    public static final int MAX_DISPLAY_HEIGHT_LAND = Math.min(MAX_DISPLAY_WIDTH_CURRENT, MAX_DISPLAY_HEIGHT_CURRENT);
    ...
    // NOTE: mSurface is a place holder of current Surface used by the
    // CodecTestBase.
    // This doesn't own the handle. The ownership with instances that manage
    // SurfaceView or TextureView, ... They hold the responsibility of calling
    // release().
    ...
    // some media types decode a pre-roll amount before playback. This would mean
    // that decoding
    // after seeking may not return the exact same values as would be obtained by
    // decoding the
    ...
    public static boolean isFeatureSupported(String name, String mediaType, String feature)
        ...
        MediaCodecInfo.CodecCapabilities codecCapabilities = codec.getCodecInfo().getCapabilitiesForType(mediaType);
    ...
    public static boolean isHDRCaptureSupported() {
        ...
        try {
            ...
            for (String id : cameraIds) {
                ...
                if (IntStream.of(caps)
                        .anyMatch(x -> x == CameraMetadata.REQUEST_AVAILABLE_CAPABILITIES_DYNAMIC_RANGE_TEN_BIT)) {
                    Set<Long> profiles = ch.get(CameraCharacteristics.REQUEST_AVAILABLE_DYNAMIC_RANGE_PROFILES)
    ...
    public static boolean doesAnyFormatHaveHDRProfile(String mediaType,
        ...
        if (profileArray != null) {
            for (MediaFormat format : formats) {
                ...
                if (IntStream.of(profileArray).anyMatch(x -> x == profile))
                    return true;
            }
        }
        return false;
    ...
    public static boolean hasSupportForColorFormat(String name, String mediaType, int colorFormat)
            throws IOException {
        MediaCodec codec = MediaCodec.createByCodecName(name);
        MediaCodecInfo.CodecCapabilities cap = codec.getCodecInfo().getCapabilitiesForType(mediaType);
    ...
    private static ArrayList<String> compileCompleteTestMediaTypesList(boolean isEncoder,
        ...
        if (mediaTypeSelKeys == null) {
            ArrayList<String> cddRequiredMediaTypesList = compileRequiredMediaTypeList(isEncoder, needAudio, needVideo);
            ...
            for (MediaCodecInfo codecInfo : codecInfos) {
                ...
                for (String type : types) {
                    ...
                    if (!needAudio && type.startsWith("audio/"))
                        continue;
                    if (!needVideo && type.startsWith("video/"))
                        continue;
            ...
            // feature_video_output is not exposed to package manager. Testing for video
            // output
            // ports, such as VGA, HDMI, DisplayPort, or a wireless port for display is also
            // not
            // direct.
            /*
    ...
    public static List<Object[]> prepareParamList(List<Object[]> exhaustiveArgsList,
        ...
        for (String mediaType : mediaTypes) {
            ArrayList<String> totalListOfCodecs = selectCodecs(mediaType, null, null, isEncoder, selectSwitch);
    ...
    protected void configureCodec(MediaFormat format, boolean isAsync,
        ...
        // signalEOS flag has nothing to do with configure. We are using this flag to
        // try all
    ...
    void validateTestState() {
         ...
         * String msg =
         * String.format("The number of output frames received is not same as number "
         * + "of input frames queued. Output count is %d, Input count is %d \n",
         * mOutputCount, mInputCount);
         * // check the pts lists to see what frames are dropped, the below call is
         * needed to
    ...
    public boolean isFormatSimilar(MediaFormat inpFormat, MediaFormat outFormat) {
        if (inpFormat == null || outFormat == null)
            return false;
        String inpMediaType = inpFormat.getString(MediaFormat.KEY_MIME);
        String outMediaType = outFormat.getString(MediaFormat.KEY_MIME);
        // not comparing input and output mediaTypes because for a codec, mediaType is
        // raw on one
        // side and encoded type on the other
        if (outMediaType.startsWith("audio/")) {
            return (inpFormat.getInteger(MediaFormat.KEY_CHANNEL_COUNT, -1) == outFormat
                    .getInteger(MediaFormat.KEY_CHANNEL_COUNT, -2))
                    && (inpFormat.getInteger(MediaFormat.KEY_SAMPLE_RATE, -1) == outFormat
                            .getInteger(MediaFormat.KEY_SAMPLE_RATE, -2))
    ...
    protected PersistableBundle validateMetrics(String codec) {
        ...
        assertEquals("error! metrics#MetricsConstants.MODE is not as expected \n" + mTestConfig
                + mTestEnv, mIsAudio ? MediaCodec.MetricsConstants.MODE_AUDIO : MediaCodec.MetricsConstants.MODE_VIDEO,
