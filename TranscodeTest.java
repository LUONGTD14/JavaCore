Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/MediaProviderTranscode/src/android/mediaprovidertranscode/cts/TranscodeTest.java blob:e50544e37ffc9928e7a82dca8b8a693909face17[file][log][blame]
/*
...
public class TranscodeTest {
    ...
    private static final File DIR_CAMERA = new File(EXTERNAL_STORAGE_DIRECTORY, Environment.DIRECTORY_DCIM + "/Camera");
    // TODO(b/169546642): Test other directories like /sdcard and /sdcard/foo
    // These are the only transcode unsupported directories we can stage files in
    // given our
    ...
    public void testNoTranscodeOutsideCamera_ConentResolver() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < noTranscodeUris.size(); i++) {
                pfdOriginal1.seekTo(0);
                ParcelFileDescriptor pfdOriginal2 = open(noTranscodeUris.get(i), false, null /* bundle */);
    ...
    public void testExtraMediaCapabilitiesHevcSupportedTrue_ContentResolver() throws Exception {
        ...
        try {
            ...
            TranscodeTestUtils.enableTranscodingForPackage(getContext().getPackageName());
            Bundle bundle = new Bundle();
            ApplicationMediaCapabilities capabilities = new ApplicationMediaCapabilities.Builder()
    ...
    public void testExtraMediaCapabilitiesHevcUnspecifiedFalse_ContentResolver() throws Exception {
        ...
        try {
            ...
            TranscodeTestUtils.enableTranscodingForPackage(getContext().getPackageName());
            Bundle bundle = new Bundle();
            ApplicationMediaCapabilities capabilities = new ApplicationMediaCapabilities.Builder().build();
            bundle.putParcelable(MediaStore.EXTRA_MEDIA_CAPABILITIES, capabilities);
            ParcelFileDescriptor pfdTranscoded = open(uri, false, bundle);
    ...
    public void testMediaCapabilitiesManifestHevc()
        ...
        try {
            ...
            // Explicitly close PFD otherwise instrumention might crash when test_app is
            // uninstalled
     ...
     * This test compares the bytes read before and after enabling transcoding for
     * the test app.
     * 
     * @throws Exception
     */
    ...
    /**
     * Tests that we never initiate tanscoding for legacy formats.
     * This test asserts using the time it took to read after enabling transcoding
     * for the test app.
     * The reason for keeping this check separately (than
     * {@link TranscodeTest#testTranscodedNotInitiatedForLegacy_UsingTiming()}) is
     * that this
     * provides a higher level of suret that the timing wasn't favorable because of
     * any caching
     * after open().
     * 
     * @throws Exception
     ...
     * For instance, due to some calculation errors we might incorrectly make
     * timeout to be 0.
     * We test this by making sure that a small HEVC video (< 1 sec long and < 1Mb
     * size) gets
     * transcoded.
     * 
     * @throws Exception
    ...
    /**
     * Tests that we transcode an HEVC file when a modern app passes the
     * mediaCapabilitiesUid of a
    ...
    public void testOriginalCallingUid_modernAppPassLegacyAppUid()
        ...
        try {
            installAppWithStoragePermissions(TEST_APP_SLOW_MOTION);
            Uri uri = TranscodeTestUtils.stageHEVCVideoFile(modernFile);
            // pfdModernApp is for original content (without transcoding) since this is a
            // modern
            // app.
            pfdModernApp = open(modernFile, false);
            // pfdModernAppPassingLegacyUid is for transcoded content since this modern app
            // is
    ...
    /**
     * Tests that we don't transcode an HEVC file when a legacy app passes the
    ...
    public void testOriginalCallingUid_legacyAppPassModernAppUid()
        ...
        try {
            ...
            // pfdLegacyAppPassingModernUid is for original content (without transcoding)
            // since this
    ...
    public void testGetOriginalMediaFormatFileDescriptor_onSuccess_returnsOriginalFileDescriptor()
        ...
        try {
            ...
            TranscodeTestUtils.enableTranscodingForPackage(getContext().getPackageName());
            ParcelFileDescriptor pfdTranscoded = open(modernFile, false);
            ParcelFileDescriptor pfdOriginalMediaFormat = MediaStore.getOriginalMediaFormatFileDescriptor(getContext(),
                    pfdTranscoded);
    ...
    /**
     * Tests that IOException is thrown from
     * {@link MediaStore#getOriginalMediaFormatFileDescriptor}
     * to indicate failure.
     * 
     * @throws Exception
    ...
    public void testTranscodingCacheClear() throws Exception {
        ...
        try {
            ...
            assertTranscode(modernFile, true);
            // Invoke PackageManager to free a huge amount of storage, so that it tries to
            // clear all
            // available caches.
     ...
     * We check this by writing something to tanscoded content and then read it
     * back.
     */
    @Test
    ...
    public void testTranscodeDirectoryNotAccessible() throws Exception {
        ...
        try {
            ...
            String transcodeDirPath = "/storage/emulated/" + UserHandle.myUserId() + "/.transforms/transcode";
    ...
    public void test_IsAppIoBlocked() throws Exception {
        ...
        try {
            ...
            // Check in a loop if app IO is blocked cos there might be a delay between
            // read(2)
