Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/tests/media/encoder/src/android/media/encoder/cts/VideoEncoderTest.java blob:bf0c71c9777fa8845da850b1361f5944be86d3d5[file][log][blame]
/*
...
public class VideoEncoderTest extends MediaTestBase {
    ...
    private static final String SOURCE_URL = mInpPrefix + "video_480x360_mp4_h264_871kbps_30fps.mp4";
    ...
    abstract class VideoProcessorBase extends MediaCodec.Callback {
        ...
        protected boolean initCodecsAndConfigureEncoder(
            ...
            VideoCapabilities encCaps = mEncoder.getCodecInfo().getCapabilitiesForType(outMime).getVideoCapabilities();
            ...
            if (mSaveResults) {
                try {
                    String outFileName = FILE_DIR + mProcessorName + "_" + bitRate + "bps";
        ...
        protected boolean fillDecoderInputBuffer(int ix) {
            ...
            while (!mSignaledDecoderEOS) {
                ...
                if (size < 0) {
                    ...
                    if (DEBUG)
                        Log.v(TAG, "queing input #" + ix + " for decoder with timestamp "
        ...
        protected void emptyEncoderOutputBuffer(int ix, BufferInfo info) {
            if (DEBUG)
                Log.v(TAG, "encoder received output #" + ix
        ...
        public boolean playBack(Surface surface) {
            return mEncodedStream.playAll(surface);
        }
    ...
    class VideoProcessor extends VideoProcessorBase {
                    ...
                    synchronized (mCondition) {
                        try {
                            ...
                            if (!haveBuffers() && !mCompleted) {
                                mCondition.wait(mEncodeOutputFormatUpdated ? FRAME_TIMEOUT_MS : INIT_TIMEOUT_MS);
                            }
        ...
        public void onOutputBufferAvailableLocked(
            ...
            if (mediaCodec == mDecoder) {
                if (DEBUG)
                    Log.v(TAG, "decoder received output #" + ix
                            + " (sz=" + info.size + ", f=" + info.flags
                            + ", ts=" + info.presentationTimeUs + ")");
                ...
                if (!mGotDecoderEOS) {
                    ...
                    synchronized (mCondition) {
                        if (ix < 0 && eos && mBuffersToRender.size() > 0) {
                            ...
                            mBuffersToRender.peekLast().second.flags |= MediaCodec.BUFFER_FLAG_END_OF_STREAM;
                        } else if (ix >= 0 || eos) {
                    ...
                    if (eos || info.size > 0) {
                        synchronized (mCondition) {
                            if (mInfoOnSurface != null || mBuffersToRender.size() > 0) {
                                if (DEBUG)
                                    Log.v(TAG, "postponing render, surface busy");
                                mBuffersToRender.addLast(Pair.create(ix, info));
                            } else {
    ...
    static class EncoderSize {
        ...
        private void initializeSizes() {
            ...
            if (DEBUG)
                Log.i(TAG, "arbitrary=" + mArbitraryW + "/" + mArbitraryH);
        }
        ...
        private void addExtremeSizesFor(int x, int y) {
                ...
                for (int dy = 0; dy <= yAlign; dy += yAlign) {
                        ...
                        if (x + y == 2 && width >= 4 * height) {
                            ...
                            width = getExtreme(
                                    mVideoCaps.getSupportedWidths()
                                            .intersect(0, wideScreen.getWidth()),
                                    x, dx);
                    ...
                    try {
                        ...
                        if (x + y == 2 && height >= 4 * width) {
                            ...
                            height = getExtreme(
                                    mVideoCaps.getSupportedHeights()
                                            .intersect(0, wideScreen.getHeight()),
                                    y, dy);
            ...
            if (DEBUG)
                Log.i(TAG, x + "x" + y + ": minMax=" + mMinMax + ", near=" + mNearMinMax);
        }
    ...
    class Encoder {
        ...
        public boolean testIntraRefresh(int width, int height) {
            ...
            for (int period : refreshPeriod) {
                Function<MediaFormat, Boolean> updateConfigFormatHook = new Function<MediaFormat, Boolean>() {
                ...
                Function<MediaFormat, Boolean> checkOutputFormatHook = new Function<MediaFormat, Boolean>() {
                    public Boolean apply(MediaFormat fmt) {
                ...
                String testName = mName + '_' + width + "x" + height + '_' + "flexYUV_intraRefresh";
                Consumer<VideoProcessorBase> configureVideoProcessor = new Consumer<VideoProcessorBase>() {
        ...
        public boolean testDetailed(
            ...
            String testName = mName + '_' + width + "x" + height + '_' + (flexYUV ? "flexYUV" : " surface");
            Consumer<VideoProcessorBase> configureVideoProcessor = new Consumer<VideoProcessorBase>() {
                public void accept(VideoProcessorBase processor) {
                    processor.setProcessorName(testName);
                }
        ...
        private boolean test(
            ...
            VideoProcessorBase processor = flexYUV ? new VideoProcessor() : new SurfaceVideoProcessor();
        ...
        for (String mediaType : mediaTypesList) {
            ...
            for (String encoder : encoders) {
                ...
                for (boolean near : new boolean[] { false, true }) {
                    Map<Size, Set<Size>> testSizes = near ? encoderSize.mNearMinMax : encoderSize.mMinMax;
                    for (int x = 0; x < 2; x++) {
                        for (int y = 0; y < 2; y++) {
