Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/tests/media/codec/src/android/media/codec/cts/EncodeVirtualDisplayWithCompositionTestImpl.java blob:9707895d e87811007f690bafec1b4e9cf53e1943[file][log][blame]
/*
...
/**
 * Impl class for tests using MediaCodec encoding with composition of multiple
 * virtual displays.
 */
public class EncodeVirtualDisplayWithCompositionTestImpl {
    ...
    private EncoderEventListener mEncoderEventListener=new EncoderEventListener(){@Override public void onCodecConfig(ByteBuffer data,MediaCodec.BufferInfo info){mCodecConfigReceived=true;}@Override public void onBufferReady(ByteBuffer data,MediaCodec.BufferInfo info){mCodecBufferReceived=true;}};
    ...
    /**
     * Run rendering test in a separate thread. This is necessary as
     * {@link OutputSurface} requires
    ...
    private void doTestRenderingOutput(final Context context, String mimeType, int w, int h,
        ...
        try {
            ...
            if (runRemotely) {
                mRemotePresentation = new RemoteVirtualDisplayPresentation(context, windowSurface, w, h);
    ...
    private boolean checkSurfaceFrameColor(int w, int h, int color) {
        // Read a pixel from the center of the surface. Might want to read from multiple
        // points
    ...
    private static boolean approxEquals(int expected, int actual) {
        // allow differences between BT.601 and BT.709 conversions during
        // encoding/decoding for now
            ...
            try {
                ...
                for (int j = 0; j < NUM_DISPLAY_CREATION; j++) {
                    ...
                    for (int k = 0; k < numDisplays; k++) {
                        virtualDisplays[k] = new VirtualDisplayPresentation(context,
    ...
    private static class GlCompositor implements SurfaceTexture.OnFrameAvailableListener {
        ...
        private class GlWindow {
             ...
             * 
             * @param uSTMatrixHandle shader handler for the STMatrix for texture
             *                        coordinates
             *                        mapping
    ...
    private static class TestPresentationBase extends Presentation {
        public TestPresentationBase(Context outerContext, Display display) {
            // This theme is required to prevent an extra view from obscuring the
            // presentation
    ...
    private static class RemoteVirtualDisplayPresentation implements Renderer {
        ...
        private final ServiceConnection mConnection = new ServiceConnection() {
            public void onServiceConnected(ComponentName arg0,IBinder arg1){mService=arg1;mConnectionWait.release();}public void onServiceDisconnected(ComponentName arg0){
        // ignore
        }};
     ...
     * Some H/Ws cannot support maximum resolution reported in encoder if decoder is
     * running
     * at the same time.
