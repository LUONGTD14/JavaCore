Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/tests/media/common/src/android/media/cts/MediaCodecTunneledPlayer.java blob:d4a8250a1ac80a0e3bc82019ed54dec88b3c4b53[file][log][blame]
/*
 ...
 * to use MediaCodec to delegate their Audio/Video rendering to a vendor
 * provided
 * Codec component.
 */
public class MediaCodecTunneledPlayer implements MediaTimeProvider {
    ...
    // Creates the extractors, identifies tracks and formats, and then calls
     ...
     *
     * @param presentationTimeOffsetUs The offset for the presentation time to start
     *                                 at.
     ...
     * In tunnel mode, retrieves the presentation timestamp of the latest rendered
     * video frame.
     * 
     * @return presentation timestamp in microseconds, or
     *         {@code CodecState.UNINITIALIZED_TIMESTAMP}
    ...
    /**
     * Returns the system time of the latest rendered frame in any of the video
     * codecs.
     */
    ...
    /**
     * Returns the ordered list of system times of rendered video frames in
     * tunnel-mode.
     *
     * Note: This assumes there is at most one tunneled mode video codec running in
     * the player.
     */
    ...
    /**
     * When the player is on stand-by, tries to queue one frame worth of video per
     * video codec.
     *
     * Returns arbitrarily the timestamp of any frame queued this way by one of the
     * video codecs.
    ...
    public AudioTimestamp getTimestamp() {
        ...
        if (mAudioCodecStates.size() != 0) {
            timestamp = mAudioCodecStates.entrySet().iterator().next().getValue().getTimestamp();
        }
        return timestamp;
    }
    ...
    /**
     * Configure an offset (in ms) to audio content to simulate track
     * desynchronization.
     */
