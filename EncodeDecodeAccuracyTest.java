Google Git Sign in android/platform/cts/refs/heads/android14-release/./tests/media/src/android/mediav2/cts/EncodeDecodeAccuracyTest.java blob:8 ba995396f31049967910006bbea244426cda734[file][log][blame]
/*
 ...
 * {@link DecoderColorAspectsTest#testColorAspects()} checks if the encoder and
 * decoder
 * components are signalling the configured color aspects correctly.
 * This test verifies if the device decoder/display is using this color aspects
 * correctly
 * <p>
 * Test pipeline:
 * 
 ...
 * </pre>
 * 
 * Assuming no quantization losses, the test verifies if the input rgb pixel
 * values and output
...
public class EncodeDecodeAccuracyTest extends CodecDecoderTestBase {
    ...
    // The bitrates are configured to large values. The content has zero motion, so
    // in-time the
    // qp of the encoded clips shall drop down to < 10. Further the color bands are
    // aligned to 2,
    // so from downsampling rgb24 to yuv420p, even if bilinear filters are used as
    // opposed to
    // skipping samples, we may not see large color loss. Hence allowable tolerance
    // is kept to 5.
    // until QP stabilizes, the tolerance is set at 7. For devices upgrading to T,
    // thresholds are
    // relaxed to 8 and 10.
    ...
    public EncodeDecodeAccuracyTest(String encoder, String mediaType,
        ...
        // aligning color bands to 2 is done because, during chroma subsampling of rgb24
        // to yuv420p,
    ...
    public static Collection<Object[]> input() {
        ...
        // Note: although vp8 and vp9 donot contain fields to signal color aspects
        // properly, this
        // information can be muxed in to containers of mkv and mp4. So even those clips
        // should pass these tests
        final String[] mediaTypes = { MediaFormat.MIMETYPE_VIDEO_AVC, MediaFormat.MIMETYPE_VIDEO_HEVC,
        ...
        for (Object[] obj : baseArgsList) {
            ...
            for (String mediaType : mediaTypes) {
                ...
                if (mediaType.equals(MediaFormat.MIMETYPE_VIDEO_VP8) && bd == 10)
                    continue;
    ...
    private void configureCodec(MediaFormat encFormat, boolean isAsync) {
        ...
        mEGLWindowInpSurface = new InputSurface(mInpSurface, false, mEncCfgParams.mInputBitDepth == 10);
        if (ENABLE_LOGS) {
    ...
    protected void dequeueOutput(int bufferIndex, MediaCodec.BufferInfo info) {
        ...
        if (info.size > 0) {
            ...
            if (checkSurfaceFrame(mOutputCount - 1))
                mBadFrames++;
        }
    }
    ...
    private void decodeElementaryStream(MediaFormat format)
        ...
        formats.add(format);
        ArrayList<String> listOfDecoders = CodecDecoderTestBase.selectCodecs(mMediaType, formats, null, false);
